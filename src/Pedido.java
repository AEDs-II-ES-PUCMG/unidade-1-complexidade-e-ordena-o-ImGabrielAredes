...
public class Pedido implements Comparable<Pedido>{
    ...
    public double indiceEconomia() {
        ...
        for (int i = 0; i < quantItens; i++) {
            if (itens[i] != null) {
                // Diferença entre o preço atual (dinâmico) e o preço congelado no pedido
                double precoAtual = itens[i].getProduto().valorDeVenda();
                double precoPago = itens[i].getPrecoVenda();
                
                economiaTotal += (precoAtual - precoPago) * itens[i].getQuantidade();
            }
        }
        return economiaTotal;
	}

		public int getVolumeTotalItens() {
        int volumeTotal = 0;
        for (int i = 0; i < quantItens; i++) {
            if (itens[i] != null) {
                volumeTotal += itens[i].getQuantidade();
            }
        }
        return volumeTotal;
    }

	    public int getCodigoPrimeiroItem() {
        if (quantItens > 0 && itens[0] != null) {
            return itens[0].getProduto().hashCode(); // hashCode é o ID na classe Produto
        }
        return 0; 
    }

	    public double getIndiceEconomia() {
        double economiaTotal = 0.0;
        for (int i = 0; i < quantItens; i++) {
            if (itens[i] != null) {
                double precoAtual = itens[i].getProduto().valorDeVenda();
